...
- **通算/日別/年度別のポイント合計**: 各半荘のポイント（差分）を**そのまま合計するだけでよい**。原点を改めて引く必要はない（`rawTotal += rawScore`。`aggregate.ts`の`computeTotals`/`computeDaySummary`/`computePlayerDailyCandles`を参照）。過去に「素点そのものを保存している」という誤った前提で `rawScore - ORIGIN_SCORE` を計算しており、半荘のたびに二重に25ポイント引かれてしまうバグがあった（実データで発覚・修正済み）
...
- `computePlayerDailyCandles(db, playerId)`: 確定済み半荘を時系列に並べ、累計ポイントの推移を**登録日単位**の四本値（始値/高値/安値/終値）に集約したもの（個人ページのローソク足チャート用。2026-09-16、`computePlayerScoreHistory`から改名・再設計。半荘単位の折れ線グラフだと上下が分かりにくいというフィードバックへの対応）。1日の中で複数半荘打っていれば、その日の増減の振れ幅（高値-安値）と最終的な方向（終値が始値以上か未満か）が分かる

共有UIコンポーネント（`src/views/components.tsx`）: `Signed`（+/-付きの数値表示。合計時の浮動小数点誤差を防ぐため小数第1位に丸める）、`Candlestick`（追加ライブラリ無しでSVGのローソク足チャートをサーバー側生成。1本＝1対局日で、始値/高値/安値/終値を`aggregate.ts`の`computePlayerDailyCandles`から受け取って描画する。終値が始値以上なら緑（陽線）、未満なら赤（陰線）。当初は半荘単位の折れ線グラフ（`Sparkline`）だったが「半荘ごとの上下だと分かりにくい」というフィードバックを受けて2026-09-16に置き換えた。グラフ自体には軸ラベルが無いため、`/players/:id`側で日付範囲・ひげ/太い部分の意味・陽線陰線の色分けを説明する文言をグラフの直前に添えている。新しく同様のグラフを使う場所でも説明文を添えること）、`TotalsTable`（プレイヤー別 ポイント合計テーブル、名前は`/players/:id`へリンク。`showHeader`prop（デフォルトtrue）でヘッダー行の表示可否を切替可能。年度別・通算タブは`showHeader={false}`で非表示にしている。`showChips`prop（デフォルトfalse）でチップ合計列の表示可否を切替可能。当日/年度別/通算タブでは非表示だが、対局日詳細の「この日の小計」だけは`showChips={true}`でチップ合計も表示する）、`TabBar`（当日/年度別/通算の3タブ、`active`と`year`をpropで指定）
