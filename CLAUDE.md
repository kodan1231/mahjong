...
- 保存時は1ブロック=1つの`game_sessions`行（`status: "confirmed"`, `displayMode: "raw"`, `memo`にラベルを保存）として登録し、`session_scores`は`rawScore`＝入力ポイント、`rankChip`＝入力チップ値をそのまま保存、**`rank`は常にnull**（着順という概念が無いため）。seqは通常行の続き番号（`maxSeq + 1`以降）を自動採番する。**このため「前半だけ小計」のように途中に小計ブロックを挟んだ場合、半荘一覧の表示順・個人ページの折れ線グラフの中間点の並び順は実際の時系列とズレることがある（最終的な合計・チップは正しい）**。厳密な時系列順が必要になった場合は別途対応を検討すること
