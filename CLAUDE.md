...
共有UIコンポーネント（`src/views/components.tsx`）: `Signed`（+/-付きの数値表示。合計時の浮動小数点誤差を防ぐため小数第1位に丸める）、`Sparkline`（追加ライブラリ無しでSVGの折れ線グラフをサーバー側生成）、`TotalsTable`（プレイヤー別 ポイント/チップ合計テーブル、名前は`/players/:id`へリンク）、`TabBar`（当日/年度別/通算の3タブ、`active`と`year`をpropで指定）
