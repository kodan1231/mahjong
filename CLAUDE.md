...
- **箱割れの判定閾値**: ポイント（差分）が`-ORIGIN_SCORE`（-25）を下回ったら、その人の素点が0未満（箱割れ）ということになる。まとめて入力では`rawScore < -ORIGIN_SCORE`で自動的に`isHakoware: true`にする。半荘確認画面では従来通りチェックボックスで手動指定（OCRでは箱割れが正しく読み取れないことがあるため）
...
共有UIコンポーネント（`src/views/components.tsx`）: `Signed`（+/-付きの数値表示。合計時の浮動小数点誤差を防ぐため小数第1位に丸める）、`Sparkline`（追加ライブラリ無しでSVGの折れ線グラフをサーバー側生成）、`TotalsTable`（プレイヤー別 ポイント合計テーブル、名前は`/players/:id`へリンク。`showHeader`prop（デフォルトtrue）でヘッダー行の表示可否を切替可能。年度別・通算タブは`showHeader={false}`で非表示にしている。`showChips`prop（デフォルトfalse）でチップ合計列の表示可否を切替可能。当日/年度別/通算タブでは非表示だが、対局日詳細の「この日の小計」だけは`showChips={true}`でチップ合計も表示する）、`TabBar`（当日/年度別/通算の3タブ、`active`と`year`をpropで指定）
