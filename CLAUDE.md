...
- `computePlayerDailyBreakdown(db, playerId, range?)`: そのプレイヤーが参加した対局日ごとの、その日1日分のポイント合計・チップ合計（役満チップ込み、日ごとに個別計算）。日付昇順で返す。個人ページの「日別」セクション（棒グラフ＋テーブル）用。2026-09-16、`computePlayerDailyCandles`（ローソク足・累計ベース）を置き換える形で追加。「見せたいのは累計の推移ではなくその日の増減」というフィードバックを受けての再設計
