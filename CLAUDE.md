...
### 点数の単位（「素点」と「ポイント」）— 何度か認識がズレて修正した経緯があるので必ず読むこと

用語を統一している:
- **素点**: 実際の麻雀の点数そのもの（例: 32000点）。点棒の実数値。
- **ポイント**: 素点を1000で割った値（例: 32000点 → 32ポイント）。**アプリ全体（個人成績・日別・年度別・通算の表示、管理画面のあらゆる入力欄、`session_scores.raw_score`の保存値）はこの「ポイント」単位で統一する。** 配給原点も25000点ではなく`ORIGIN_SCORE = 25`（ポイント）。素点は100点単位で丸められるため、ポイントは小数第1位までの小数になりうる（例: 24700点 → 24.7ポイント）。`session_scores.raw_score`は`real`型（小数対応）。管理画面の数値入力欄には`step="0.1"`を付けている
- **例外は写真撮影（OCR）のときだけ**: 点数表示機の実機表示は2パターンある。①「素点そのまま表示」モード → 表示されている数値は「素点」（例: 32000）なので、アプリに取り込む際に1000で割ってポイントに変換する。②「集計結果（配給原点からの差分）表示」モード → 表示されている数値（例: "+7"）は既にポイント単位の差分なので、そのままorigin(25)に加算するだけでよい（1000倍もしない、割り算もしない）。この変換は`src/lib/scoring.ts`の`normalizeRawScore(ocrValue, displayMode, origin)`が担う:
  ```ts
  return displayMode === "diff" ? origin + ocrValue : Math.round(ocrValue / 100) / 10;
  ```
  **rawモードとdiffモードで換算方法が逆になる**点を必ず理解してから触ること（過去に両方向で間違えたことがある）。手入力（半荘確認画面・まとめて入力画面）は常にポイント単位でそのまま受け取る（変換不要）
...
- **箱割れ**: 電子表示機はマイナスを反映しないことがあるため、管理者が該当プレイヤーを「箱割れ」としてマークし、実際のマイナスポイントを手動入力して上書きできる
- **年間合計ポイントの基準点**: 25ポイント（素点25000点持ち）。素点ではなく必ずポイント単位で計算する（`rawTotal = rawScore - ORIGIN_SCORE`）
...
共有UIコンポーネント（`src/views/components.tsx`）: `Signed`（+/-付きの数値表示。合計時の浮動小数点誤差を防ぐため小数第1位に丸める）、`Sparkline`（追加ライブラリ無しでSVGの折れ線グラフをサーバー側生成）、`TotalsTable`（プレイヤー別 ポイント合計テーブル、名前は`/players/:id`へリンク。`showHeader`prop（デフォルトtrue）でヘッダー行の表示可否を切替可能。年度別・通算タブは`showHeader={false}`で非表示にしている）、`TabBar`（当日/年度別/通算の3タブ、`active`と`year`をpropで指定）
...
- **同点行の扱い**: 保存はするが`status: "pending"`のまま（rank/rankChipは付けない）にし、`?tied=3,4`で警告＋「同点の回を入力順で仮に確定する」チェックボックスを表示する。チェックして再送信すると`acceptTies=1`が送られ、入力順（列の並び＝左ほど上位）で`computeRankAndChips`のタイブレーク結果をそのまま採用してconfirmedにする。単一半荘の確認画面（`/days/:id/sessions/:sid/confirm`）にも同じ仕組み（`acceptTie`パラメータ）がある
- **箱割れの自動判定**: チェックボックスは無く、入力したポイントが0未満ならその場で`isHakoware: true`として保存する（雀荘の点棒は0未満を表現できないため、バックフィル時は素点そのままマイナスで入力する運用）
- 役満・局メモはこの画面では扱わない。保存後に`/days/:id`や個別半荘の確認画面から編集する
- display_modeは常に`raw`固定（ポイントをそのまま入力する前提。手入力は変換不要）
