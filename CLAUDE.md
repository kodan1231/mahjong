...
- **座席の表記は「座席N」ではなく起家/南家/西家/北家**（2026-09-16変更。「座席1、2、3ではなく起家、南家、西家、北家の表現がいい」というフィードバック）: `WIND_LABELS = ["起家","南家","西家","北家"]`（`src/routes/days.tsx`冒頭）で`seatIndex`0-3を対応付ける。座席は半荘開始時に1回だけ登録し（自風が半荘中に変わっても座席登録自体はやり直さない）、以後その半荘では常にこの対応関係で扱う。座席登録画面（`/days/:id/sessions/new`）・確認画面（`/days/:id/sessions/:sid/confirm`）のラベル、および半荘一覧の表（`DayDetailBody`）に追加した「風」列で使用。半荘一覧の各行はもともと`seatIndex`昇順で並んでいた（`rows.sort((a,b)=>a.seatIndex-b.seatIndex)`）ため、この対応付けだけで自動的に起家→南家→西家→北家の順で表示される。小計ブロック（`seatIndex`が4以上になり得る）では`WIND_LABELS[r.seatIndex]`が`undefined`になるため`?? "-"`で表示する
