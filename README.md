...
2. （管理者）ナビの「対局日を開始」でその日の参加者を選び対局日を開始（同時に開始できる対局日は1つまで）
3. （管理者）半荘開始前に「次の半荘を登録」でその半荘の座席順（管理者から見た並び＝点数表示機の表示順）を登録
4. （管理者）半荘終了後、点数表示機を撮影 → OCR結果を確認・修正して確定（箱割れがあれば手動で実際のマイナス点数に上書き）
5. （管理者）役満が出たら役名と和了者を登録。局ごとの和了メモも任意で追加可能
6. （管理者）その日の対局が終わったらナビの「対局日を終了」で締める
7. （全員）トップページは「当日」「年度別」「通算」の3タブ。年度別タブから対局日をクリックすると詳細が見られ、プレイヤー名をクリックするとその人の個人成績（通算合計・推移グラフなど）が見られる
